// client/src/FriendQuiz.jsx — FriendQuiz (full)
...
function CreateQuizScreen({creatorName,onBack,onDone}) {
  ...
  async function handleNext() {
    ...
    setSaving(true);
    try{const res=await createQuiz({creatorName,answers});onDone(res);}
    catch(e){alert("Error: "+e.message);}
    finally{setSaving(false);}
  }
  return (
    <PhotoSlide question={q} index={current} total={QUESTIONS.length}>
      <p style={{fontFamily:"'Nunito',sans-serif",fontSize:"0.78rem",fontWeight:800,letterSpacing:"2px",textTransform:"uppercase",color:"rgba(255,255,255,0.5)",marginBottom:8}}>{q.label}</p>
      <h2 style={{fontFamily:"'Fredoka One',cursive",fontSize:"clamp(1.35rem,5vw,1.8rem)",color:"#fff",marginBottom:18,lineHeight:1.25,textShadow:"0 2px 12px rgba(0,0,0,0.4)"}}>Your {q.label.toLowerCase()}?</h2>
      <input className="fq-input" type="text" placeholder="Type your answer…" value={val} onChange={e=>setAnswers(p=>({...p,[q.key]:e.target.value}))} onKeyDown={e=>{if(e.key==="Enter"&&val.trim())handleNext();}} style={{marginBottom:16}} autoFocus/>
      <Row>
        {current>0&&<BackBtn onClick={()=>setCurrent(c=>c-1)}/>}
        <button className="fq-btn fq-btn-primary" onClick={handleNext} disabled={!val.trim()||saving} style={{flex:1}}>{saving?"Saving…":current===QUESTIONS.length-1?"✅ Finish & Create Link":"Next →"}</button>
      </Row>
    </PhotoSlide>
  );
}
...
function ProfileCreatedScreen({creatorName,quizCode,onViewDashboard}) {
  ...
  function copy(){navigator.clipboard.writeText(url).catch(()=>{});setCopied(true);setTimeout(()=>setCopied(false),2500);}
  return (
    <DarkScreen style={{justifyContent:"flex-start",paddingTop:"3rem"}}>
      ...
      <div style={{background:"rgba(255,255,255,0.07)",border:"2px dashed rgba(255,255,255,0.22)",borderRadius:20,padding:"20px",marginBottom:14,textAlign:"center"}}>
        ...
        <p style={{fontFamily:"'Nunito',sans-serif",fontSize:"0.82rem",color:"rgba(255,255,255,0.8)",wordBreak:"break-all",marginBottom:14}}>{url}</p>
        <div style={{display:"inline-block",fontFamily:"'Fredoka One',cursive",fontSize:"1.6rem",color:"#fff",background:"rgba(124,58,237,0.35)",border:"2px solid rgba(124,58,237,0.55)",borderRadius:14,padding:"6px 24px",letterSpacing:5,marginBottom:12}}>{quizCode}</div>
      ...
      <div style={{maxWidth:440,margin:"0 auto"}}>
        ...
        {permission==="default"&&(
          <div style={{background:"rgba(245,158,11,0.15)",border:"1px solid rgba(245,158,11,0.4)",borderRadius:16,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
            ...
            <button onClick={requestPermission} style={{background:"rgba(245,158,11,0.3)",border:"1px solid rgba(245,158,11,0.5)",borderRadius:10,padding:"6px 14px",color:"#fff",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.8rem",cursor:"pointer",whiteSpace:"nowrap"}}>Allow</button>
          </div>
        )}
        {permission==="denied"&&(
          <div style={{background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:16,padding:"12px 16px",marginBottom:16}}>
            <p style={{fontFamily:"'Nunito',sans-serif",fontSize:"0.82rem",color:"rgba(255,255,255,0.6)"}}>🚫 Notifications blocked. Click the 🔒 lock icon in your address bar → Notifications → Allow.</p>
          </div>
        )}
        {permission==="granted"&&(
...
function PlayerIntroScreen({quiz,onBack,onStart}) {
  ...
  return (
    <DarkScreen>
      ...
      <FQInput value={name} onChange={setName} placeholder="Your name…" autoFocus/>
...
function PlayScreen({quiz,playerName,relation,onFinish}) {
  ...
  return (
    <PhotoSlide question={q} index={current} total={QUESTIONS.length}>
      <p style={{fontFamily:"'Nunito',sans-serif",fontSize:"0.78rem",fontWeight:800,letterSpacing:"2px",textTransform:"uppercase",color:"rgba(255,255,255,0.5)",marginBottom:8}}>{q.label}</p>
      <h2 style={{fontFamily:"'Fredoka One',cursive",fontSize:"clamp(1.3rem,5vw,1.75rem)",color:"#fff",marginBottom:18,lineHeight:1.25,textShadow:"0 2px 12px rgba(0,0,0,0.4)"}}>{q.question.replace("{name}",quiz.creator_name)}</h2>
      <input className="fq-input" type="text" placeholder="Type your answer…" value={val} onChange={e=>setPlayerAnswers(p=>({...p,[q.key]:e.target.value}))} onKeyDown={e=>{if(e.key==="Enter"&&val.trim())handleNext();}} style={{marginBottom:16}} autoFocus/>
      <Row>
        {current>0&&<BackBtn onClick={()=>setCurrent(c=>c-1)}/>}
        <button className="fq-btn fq-btn-primary" onClick={handleNext} disabled={!val.trim()||submitting} style={{flex:1}}>{submitting?"Submitting…":current===QUESTIONS.length-1?"✅ Submit Answers":"Next →"}</button>
      </Row>
    </PhotoSlide>
  );
}
    ...
    <div style={{height:"100%",background:`linear-gradient(145deg,${C.dark},#1a2744)`,overflowY:"auto",padding:"2rem 1rem 2.5rem"}}>
      ...
      <div style={{maxWidth:420,margin:"0 auto",animation:"fadeUp 0.5s ease"}}>
        ...
        <div style={{background:"rgba(16,185,129,0.15)",border:"1px solid rgba(16,185,129,0.35)",borderRadius:16,padding:"12px 16px",marginBottom:18,display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:"1.4rem"}}>📨</span>
          <p style={{fontFamily:"'Nunito',sans-serif",fontSize:"0.88rem",color:"rgba(255,255,255,0.75)",lineHeight:1.5}}>Your score has been sent to <strong style={{color:"#fff"}}>{quiz.creator_name}</strong>!</p>
        </div>
...
export default function FriendQuiz() {
  ...
  return (
    <div style={{height:"100%"}}>
      ...
      {screen==="creator-name" && <CreatorNameScreen onBack={()=>go("home")} onNext={name=>{setCreatorName(name);go("create-quiz");}}/>}
      {screen==="create-quiz" && <CreateQuizScreen creatorName={creatorName} onBack={()=>go("creator-name")} onDone={meta=>{setQuizMeta(meta);go("profile-created");}}/>}
      ...
      {screen==="play"      &&activeQuiz && <PlayScreen quiz={activeQuiz} playerName={playerName} relation={relation} onFinish={result=>{setFinalResult(result);go("results");}}/>}
      {screen==="results"   &&activeQuiz&&finalResult && <ResultsScreen quiz={activeQuiz} playerName={playerName} relation={relation} result={finalResult} onHome={()=>{go("home");setActiveQuiz(null);setFinalResult(null);}}/>}
    </div>
  );
}
